"""Scan rawdata layer parquets: footer metadata + file stats per (source, layer)."""
...
def scan_rawdata() -> list[dict[str, Any]]:
    """One row per parquet found under data/rawdata/<source>/."""
    rows: list[dict[str, Any]] = []
    for source in SOURCES.values():
        rawdata_dir = source.rawdata_dir
        if not rawdata_dir.exists():
            continue
        for parquet in sorted(rawdata_dir.glob("*.parquet")):
            try:
